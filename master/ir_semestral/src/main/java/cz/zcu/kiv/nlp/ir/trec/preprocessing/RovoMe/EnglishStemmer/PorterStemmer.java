...
import cz.zcu.kiv.nlp.ir.trec.preprocessing.Stemmer;
...
public final class PorterStemmer implements Stemmer
{
     ...
     *
     * @param word
     *         The word that should be transformed in a certain principal part
     *
    ...
    /**
     * Step 1 of Porter's algorithm.
     * <p>
     * This step deals with plurals and forming it back to singular. Furthermore it turns ending 'y's to 'i' when there
     * is another vowel in the stem.
     *
     * @param word
     *         The word which should be stemmed
     *
    ...
    private static String step1(String word)
    {
        ...
        if (word.endsWith("s"))
        {
            ...
            else if (word.endsWith("s") && !word.endsWith("ss")) // S -->   but SS --> SS
            {
                word = replace(word, "s", "");
            }
        }
        ...
        LOG.debug("After step 1: {}", word);
        return word;
    }

    /**
     * Step 2 of Porter's algorithm.
     * <p>
     * Maps double suffices to single ones. So -ization (= -ize plus -ation) maps to -ize etc. Note that the measure of
     * the string before the suffix (= stem) must be > 0.
     * <p>
     * Includes later changes of some rules <ul> <li>(m > 0) LOGI --> LOG added</li> <li>(m > 0) ABLI --> ABLE changed
     * to (m > 0) BLI --> BLE</li> </ul>
     *
     * @param word
     *         The word which should be stemmed
     *
    ...
    private static String step2(String word)
    {
        ...
        LOG.debug("After step 2: {}", word);
        return word;
    }

    /**
     * Step 3 of Porter's algorithm.
     * <p>
     * Deals with -ic-, -full, -ness etc. Similar strategy to step 2.
     *
     * @param word
     *         The word which should be stemmed
     *
    ...
    private static String step3(String word)
    {
        ...
        LOG.debug("After step 3: {}", word);
        return word;
    }

    /**
     * Step4 of Porter's algorithm.
     * <p>
     * Takes off -ant, -ence etc., in context [c]vcvc[V]
     *
     * @param word
    ...
    private static String step4(String word)
    {
        ...
        LOG.debug("After step 4: {}", word);
        return word;
    }

    /**
     * Step 5 of Porter's algorithm.
     * <p>
     * Removes a final -e if the measure of the stem is > 1.
     *
     * @param word
    ...
    private static String step5(String word)
    {
        ...
        else if (word.endsWith("e") && !endsWith_cvc(stemWord(word, "e")) &&
                 getMeasure(stemWord(word, "e")) == 1) // (m=1 and not *o) E -->
        {
            word = replace(word, "e", "");
        }
        // steb 5b
        else if (word.endsWith("l") && hasDoubleConsonantEnding(word) &&
                 getMeasure(word) > 1) // (m>1 and *d and *L) --> single letter
        {
            word = word.substring(0, word.length() - 1);
        }

        LOG.debug("After step 5: {}", word);
        return word;
    }


    /**
     * Returns the stem of a word based on its suffix.
     *
     * @param word
     *         Word including stem and suffix
     * @param suffix
     *         Suffix which should get removed
     *
    ...
    /**
     * Replaces the old ending of a word with a new ending.
     *
     * @param word
     *         Word which ending should be modified
     * @param oldEnd
     *         The old ending that should be replaced
     * @param newEnd
     *         The new ending of the word
     *
     * @return The new word after the replacement
     *
     * @throws IllegalArgumentException
     *         if the old ending is larger then the word itself
     * @throws IllegalArgumentException
     *         if the old ending could not be found inside the word itself
     */
    ...
    /**
     * Checks if a word stem contains a vowel.
     *
     * @param stem
     *         Stem of a word that should be examined for occurrence of at least one vowel
     *
    ...
    /**
     * Decides if a word ends on the same two consonant letters - not vowels!
     *
     * @param word
     *         The word which last two letters should be examined for equality and for being consonants
     *
     * @return true if the word ends on the same two consonant letters; false if either the two characters are different
     * or the characters are vowels
     */
    private static boolean hasDoubleConsonantEnding(final String word)
    {
        ...
        // check if the last two chars (which are equal) are vowels
        // Note: as a Y following a consonant turns into a vowel
        // double Ys aren't possible as double consonants!
        return isConsonant(txt[length - 1], txt[length - 2]);

    }

    /**
     * Decides if the last three characters of a word are of the form consonant-vowel-consonant and the last consonant
     * is neither 'W', 'X' nor 'Y'.
     *
     * @param word
     *         The word which last tree letters should be examined
     *
     * @return true if the word ends with a sequence of consonant-vowel-consonant; false otherwise
     */
    ...
    private static boolean isNonOf_wxy(char c)
    {
        return c != 'w' && c != 'x' && c != 'y';
    }

    /**
     * Decides if a char is a consonant or a vowel.
     * <p>
     * This function is intended for characters at the start of a word as no preceding character is available, so only
     * 'a', 'e', 'i', 'o' and 'u' are considered to be vowels.
     * <p>
     * If 'y' should be considered a vowel too, please use {@link #isConsonant(char, char)} instead.
     *
     * @param c
     *         Character to decide if it is a consonant or a vowel
     *
    ...
    /**
     * Decides if a char is a consonant or vowel.
     * <p>
     * The previous char is necessary as 'Y' characters turn into a vowel if a preceding character is a consonant!
     * <p>
     * Use {@link #isConsonant(char)} if 'y' should always considered to a consonant.
     *
     * @param c
     *         Character to decide if it is a consonant or a vowel
     * @param prevChar
     *         Preceding character of c
     *
     * @return true if the character is a consonant, false if it is a vowel
     */
    private static boolean isConsonant(final char c, final char prevChar)
    {
        return !(!isConsonant(c) || c == 'y' && isConsonant(prevChar));
    }

    /**
     * Converts a word into a {@link List} of {@link Letter}s in exact {@link Vowel}s and {@link Consonant}s.
     * <p>
     * Each {@link Letter} object will contain either a sequence of vowels or consonant but never both of them!
     * <p>
     * The {@link List} itself contains the {@link Letter} objects in an unsorted order which keeps the structure of the
     * origin word as it is.
     *
     * @param word
     *         The word which should get converted
     *
     * @return A {@link List} of {@link Letter}s containing sequences of {@link Vowel}s and {@link Consonant}s
     */
    ...
    /**
     * Calculates the measure of a word according to Porter's definition in his paper 'An algorithm for suffix
     * stripping".
     * <p>
     * It therefore divides {@link Letter}s into {@link Vowel}s and {@link Consonant}s which he uses to calculate the
     * measure for a word <ul> <li>Vowel v: A, E, I, O, U, Y when preceding char is a consonant</li> <li>Consonant c:
     * Everything except v</li> </ul>
     * <p>
     * Further a series of consonants and vowels can be shortened as <code>ccc...vvv...ccc...vvv --> CVCV</code>
     * <p>
     * The calculation of the measure can now be expressed as <code>[C](VC)^m[V]</code> where m is the measure of a word
     * with the letters in the square brackets being optional elements <ul>Examples: <li>m=0: TR, EE, TREE, Y, BY</li>
     * <li>m=1: TROUBLES, OATS, TREES, IVY</li> <li>m=2: TROUBLES, PRIVATE, OATEN, ORRERY</li> </ul>
     *
     * @param stem
     *         The stem of the word which measure should be calculated
     *
